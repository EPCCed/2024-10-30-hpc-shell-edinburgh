...
```bash
$ ssh-keygen -o -a 100 -t rsa -b 4096 -f ~/.ssh/id_`r config$remote$name`_rsa
```
...
new files: your private key (`~/.ssh/key_`r config$remote$name`_rsa`) and
the public key (`~/.ssh/key_`r config$remote$name`_rsa.pub`). If a key is
...
workshop, we will connect to `r config$remote$name` --- an HPC system
located at the University of Edinburgh. Although it's unlikely that
every system will be exactly like `r config$remote$name`, it's a very good
...
```bash
ssh -i ~/.ssh/key_`r config$remote$name`_ed25519 yourUsername@`r config$remote$login`
```

or

```bash
ssh -i ~/.ssh/key_`r config$remote$name`_rsa yourUsername@`r config$remote$login`
```

or if SSH keys have not been enabled

```bash
ssh yourUsername@`r config$remote$name`
```

```{r, child=paste(snippets, '/login_output.Rmd', sep=''), eval=TRUE}
```
...
you see a change from `[local]$` to `r config$remote$prompt` and
