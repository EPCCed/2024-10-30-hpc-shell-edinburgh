...
3. Choose the ``r config$sched$project`` project “Choose Project for Machine Account” box and click "Next"
...
```bash
--qos=`r config$sched$qos` --reservation=`r config$sched$reservation`
```
you should replace them with:

```bash
--qos=short --reservation=`r config$sched$qos`
```
...
#SBATCH --reservation=`r config$sched$qos`
```
